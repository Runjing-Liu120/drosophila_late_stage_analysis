# these functions copied form
# https://github.com/karlkumbier/drosophilaUtilities
...
fitBeta <- function(y, Dstd) {
  require('glmnet')
  cv.fit <- cv.glmnet(x=Dstd, y=y, family='gaussian', nfolds=10, nlambda=100,
                      lower.limits=0, intercept=TRUE, type.measure='mse',
                      standardize=FALSE, keep=FALSE)
  lambda.opt <- which(cv.fit$lambda == cv.fit$lambda.1se)
  betas <- cv.fit$glmnet.fit$beta[,lambda.opt]
  return(betas)
}
